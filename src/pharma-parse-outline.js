// Parser for the Full AI Toolkit for Work and Side Income outline (v2) — quiz-pipeline side.
...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    // All four objectives sit in ONE bulleted, <w:br/>-separated paragraph:
    //   "• LO1: Analyze …⏎• LO2: Analyze and compare …"
    // so each line is matched separately rather than the paragraph as a whole.
    if (!inPart2 && /(?:^|\n)\s*(?:•\s*)?LO\d+\s*[:\-–]/i.test(t)) {
      let hit = false;
      for (const line of t.split('\n')) {
        const lm = line.match(/^\s*(?:•\s*)?(LO\d+)\s*(?:\([^)]*\))?\s*[:\-–]\s*(.+)$/i);
        if (lm) { course.los[lm[1].toUpperCase()] = lm[2].trim(); hit = true; }
      }
      if (hit) continue;
    }
    ...
    // "Aligned Learning Objective: LO1, LO2" — this course aligns TWO objectives to each
    // module where the others align one, so every id on the line is captured. The builder
    // already prefers `alignedLOs` over `alignedLO` when both exist.

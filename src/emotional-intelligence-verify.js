...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'emotional-intelligence', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors emotional-intelligence-build.js. This source carries the verdict inside the explanation's
// letter label ("B (Correct): …"), which the parser already consumed, so in practice there is
// nothing left to strip; the guard exists for the same reason it does in the builder, and the
// two must stay identical or the comparison below would fail on a document that is correct.
...
let fail = 0;
// Verbatim prompt lines the importer is known to mishandle. Not failures — the course owner
// chose them — but echoed at the end so shipping them is never silent.
const atRisk = [];
const bad = m => { console.log('   FAIL: ' + m); fail++; };

for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Emotional_Intelligence.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // One prompt paragraph, with the word "Scenario" kept and its colon re-punctuated as
    // "Scenario — ". Mirrors emotional-intelligence-build.js; the two must stay identical or this fails a
    // correct document. See the note there for the three uploads that settled the form.
    ...
    // No prompt line may begin with a ONE-WORD "Label:" — the importer reads that as an answer
    // option. A multi-word opening that happens to contain a colon is prose; see the note in
    // emotional-intelligence-build.js. It is reported, not failed.
    if (gotPrompt.length > 1) atRisk.push(`${tag}: ${gotPrompt.length} prompt paragraphs`);
    gotPrompt.forEach((p, i) => {
      const bare = /^([A-Za-z][A-Za-z ]{0,24}):\s*$/.exec(p);
      const label = /^([A-Za-z][A-Za-z ]{0,24}):\s/.exec(p);
      const one = (bare && !/\s/.test(bare[1])) ? bare : (label && !/\s/.test(label[1])) ? label : null;
      if (one) {
        atRisk.push(`${tag}: prompt line ${i + 1} opens "${one[1]}:"`);
        console.log(`   NOTE ${tag} prompt line ${i + 1} opens the one-word label "${one[1]}:" — kept `
          + 'verbatim by decision; the importer reads such a line as an answer option');
      } else if (label)
        console.log(`   NOTE ${tag} prompt opens "${label[1]}:" — prose, not a label; import-test it`);
      if (/^\s|\s$/.test(blk[1 + i] || ''))
    ...
    const shape = blk.map(l =>
      ...
      : /^Feedback:\s/.test(l) ? 'F'
      : 'P');
    // Always 1: the builder joins every prompt to a single paragraph. A shape of HPP… is an
    // unmatched line and breaks the parse for the whole document.
    const nPromptParas = 1;

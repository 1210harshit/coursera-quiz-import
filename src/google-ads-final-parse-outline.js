// Parser for the Google Ads for Performance Marketers outline — quiz-pipeline side, final
// assessment. Byte-identical outline to the `google-ads` slug; this exists so the two
// assessments can be built independently without either reaching into the other's work dir.
...
//
// One addition over every other outline parser: `index`, a normalised video-title ->
// M<x>L<y>V<z> lookup. This quiz states its mapping as "Source video: <title>" rather than
// as a code, so google-ads-parse-quiz.js resolves titles through this index. Titles are
// normalised only for matching — `map[key].video` keeps the outline's exact wording, which
// is what the reference line prints.
...
for (const [t, keys] of Object.entries(index)) {
  if (keys.length > 1) warn.push(`video title used by ${keys.join(', ')}: "${t}" — quiz mapping will be ambiguous`);
}

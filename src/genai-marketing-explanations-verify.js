...
const mods = JSON.parse(fs.readFileSync(path.join(SP, 'genai-marketing-explanations', 'quiz.json'), 'utf8'));
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'genai-marketing', 'outline.json'), 'utf8'));
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_GenAI_Marketing_v2.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    src.options.forEach((o, oi) => {
      ...
      if (fbs[oi] !== wantFb) {
        ...
        // Only a standalone verdict LABEL is a defect. "Incorrect because ..." is a real
        // sentence in this source and must survive.
        if (/^Feedback:\s*(Correct|Incorrect)[.:,!]\s/i.test(fbs[oi]))
...
console.log(fail === 0
  ? '\n✅ ALL CHECKS PASSED — 4 files, 40 questions, 160 feedback blocks, references verified against the outline.'
  : `\n❌ ${fail} CHECK(S) FAILED`);

...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'google-ads-final', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors build.js: removes a leading verdict marker in either the bare or bracketed form and
// the whitespace after it. All other authored spacing is preserved verbatim.
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Google_Ads_Final.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    src.options.forEach((o, oi) => {
      ...
      if (fbs[oi] !== wantFb) {
        ...
        // no leftover verdict marker or label at the start of the explanation
        if (/^Feedback:\s*\(?(Correct|Incorrect)\)?\b/i.test(fbs[oi]))
          bad(`${tag} feedback ${o.letter}: verdict still present -> "${fbs[oi].slice(0, 50)}"`);

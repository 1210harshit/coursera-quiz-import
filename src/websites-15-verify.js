...
const { map: VIDEOS, meta: MMETA } =
  JSON.parse(fs.readFileSync(path.join(SP, 'websites-15', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors build.js: removes a bracketed "(Correct)" / "(Incorrect)" marker and the whitespace
// after it. All other authored spacing is preserved verbatim.
const stripMarker = fb => String(fb).replace(/^\((?:Correct|Incorrect)\)\s*/i, '');

// Mirrors build.js: a video mapping names the video, a lesson-only mapping names the lesson.
const referLine = m => {
  if (/^M\d+L\d+V\d+$/.test(m)) return `Refer to ${m}: ${(VIDEOS[m] || {}).video}`;
  const lm = /^M(\d+)L(\d+)$/.exec(m) || [];
  const les = ((MMETA['M' + lm[1]] || { lessons: {} }).lessons || {})[+lm[2]] || {};
  return `Refer to ${m}: ${les.title}`;
};
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Websites_15.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    src.options.forEach((o, oi) => {
      ...
      const wantFb = 'Feedback: ' + stripMarker(src.feedback[o.letter]) + ' (' + wantRef + ')';
      if (fbs[oi] !== wantFb) {
        ...
        // A video mapping ends V<n>; a lesson-only mapping stops at L<n>. Both are legal here,
        // and the code in the line must be the one quiz.json holds — not a level up or down.
        if (!/^Refer to M\d+L\d+(V\d+)?: \S/.test(wantRef))
          bad(`${tag} feedback ${o.letter}: bad reference line "${wantRef}"`);
        if (!wantRef.startsWith(`Refer to ${src.mapped}: `))
          bad(`${tag} feedback ${o.letter}: reference names ${wantRef.split(':')[0].replace('Refer to ', '')}, `
            + `quiz.json says ${src.mapped}`);
        if (src.mappedLevel === 'video' && !/V\d+$/.test(src.mapped))
          bad(`${tag}: mappedLevel is "video" but ${src.mapped} names no video`);
        if (src.mappedLevel === 'lesson' && /V\d+$/.test(src.mapped))
          bad(`${tag}: mappedLevel is "lesson" but ${src.mapped} names a video`);
        ...
        // no leftover (Correct)/(Incorrect) marker anywhere in the explanation — not just at
        // the start, since a marker mid-sentence would mean the source put two in one cell
        if (/\((?:Correct|Incorrect)\)/i.test(fbs[oi]))
          bad(`${tag} feedback ${o.letter}: (Correct)/(Incorrect) marker still present -> "${fbs[oi].slice(0, 50)}"`);

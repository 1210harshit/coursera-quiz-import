...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'google-ads', 'outline.json'), 'utf8'));
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Google_Ads.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // This source states one explanation per question, which the parser repeats across all
    // four options. Assert that it really is repeated, so a future parser change that starts
    // emitting per-option text cannot land here unnoticed.
    if (src.feedbackShared) {
      const explanations = new Set(fbs);
      if (fbs.length === 4 && explanations.size !== 1)
        bad(`${tag} feedbackShared is set but the four Feedback: lines differ`);
    }

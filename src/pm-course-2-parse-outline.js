...
const SP = __dirname;
const xml = fs.readFileSync(path.join(SP, 'pm-course-2', 'outline', 'word', 'document.xml'), 'utf8');
...
let inCourse2 = false, mod = null, les = null;
...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    if (/^Course\s*2\b/i.test(t)) { inCourse2 = true; mod = null; les = null; continue; }
    if (/^Course\s*[3-9]\b/i.test(t)) break;          // Course 2 only
    if (!inCourse2) continue;
    ...
    if ((m = t.match(/^C2LO(\d+)\s*:\s*(.+)$/i))) { course.los['C2LO' + m[1]] = m[2].trim(); continue; }
    ...
    if ((m = t.match(/^Aligned Course-level Learning Objective\s*:\s*(C2LO\d+)/i)) && mod) {

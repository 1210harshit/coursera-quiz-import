// Paid Advertising Across 11 Platforms (outline v2) — outline .docx -> course.json for the
// course-content importer.
//
// The same source shape as ai-toolkit, google-ads and paid-social: bare "Module N" /
// "Lesson N" headings with the name on a following "Title of the Module:" line, video
// descriptions prefixed with a literal "Description: " label, "Aligned Learning Objective: LO4"
// stating only the id, and a Lead Instructor still set to the template placeholder.
//
// This is the 11-platform successor to paid-social's nine, so the objective ids reach two
// digits (LO10, LO11) — the id patterns here are all \d+ rather than a single digit for that
// reason. Reading rows are labelled "Reading (1)" and priced "5 mins each", so the count comes
// from the label; the two course-level videos are priced "<=4 mins" and "<=2 mins".
//

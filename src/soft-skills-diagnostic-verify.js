...
const { meta: MMETA } = JSON.parse(fs.readFileSync(path.join(SP, 'soft-skills', 'outline.json'), 'utf8'));
...
// soft-skills-diagnostic-build.js. The two must produce byte-identical lines.
const referLine = n => {
  const v = MMETA['M' + n];
...
for (const mo of mods) {
  const file = path.join(OUT, 'Coursera_Import_Pre_Course_Diagnostic_Soft_Skills.docx');
  ...
  idx.forEach((start, k) => {
    ...
    src.options.forEach((o, oi) => {
      ...
      if (fbs[oi] !== wantFb) {
        ...
        // A module title may itself contain brackets or a colon ("Putting Soft Skills to Work:
        // Ethic, Leadership, and Career"), so the wrapper may legitimately nest and the title is
    ...
    // One paragraph, joined with a space — mirrors soft-skills-diagnostic-build.js, which must

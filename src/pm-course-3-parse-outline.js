...
const xml = fs.readFileSync(path.join(SP, 'pm-course-3', 'outline', 'word', 'document.xml'), 'utf8');
...
let inCourse3 = false, mod = null, les = null;
...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    if (/^Course\s*3\b/i.test(t)) { inCourse3 = true; mod = null; les = null; continue; }
    if (/^Course\s*[4-9]\b/i.test(t)) break;          // Course 3 only
    if (!inCourse3) continue;
    ...
    if ((m = t.match(/^C3LO(\d+)\s*:\s*(.+)$/i))) { course.los['C3LO' + m[1]] = m[2].trim(); continue; }

    // "Introduction to Course 1" and similar are not lesson content
    if (/^Introduction to Course/i.test(t)) { mod = null; les = null; continue; }

    // Course 2 writes the module title inline ("Module 1: Stakeholder Identification…");
    // Course 1 used a separate "Title of the Module:" line. Accept both.
    if ((m = t.match(/^Module\s+(\d+)\s*(?::\s*(.+))?$/i))) {
      mod = +m[1]; les = null;
      meta['M' + mod] = meta['M' + mod] || { title: '', lessons: {} };
      if (m[2]) meta['M' + mod].title = m[2].trim();
      continue;
    }
    if ((m = t.match(/^Title of the Module\s*:\s*(.+)$/i)) && mod) { meta['M' + mod].title = m[1].trim(); continue; }
    if ((m = t.match(/^Aligned Course-level Learning Objective\s*:\s*(C3LO\d+)/i)) && mod) {

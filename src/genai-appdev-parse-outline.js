...
const xml = fs.readFileSync(require('path').join(__dirname,'genai-appdev','outline','word','document.xml'), 'utf8');
...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    // "Lead Instructor:" is blank in this outline; the bio paragraph that follows the
    // "Instructor Bio:" heading opens with the name.
    if (/^Instructor Bio\s*:?\s*$/i.test(t)) { course._expectBio = true; }
    else if (course._expectBio) {
      const bm = t.match(/^([A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){0,3})\s+is\b/);
      if (bm) course.instructorFromBio = bm[1].trim();
      course._expectBio = false;
    }
    if ((m = t.match(/^LO(\d+)\s*:\s*(.+)$/))) course.los['LO' + m[1]] = m[2].trim();
    if ((m = t.match(/^Aligned Learning Objective\s*:\s*(LO\d+)/)) && mod)

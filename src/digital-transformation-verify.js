...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'digital-transformation', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors digital-transformation-build.js. This source carries the verdict inside the explanation's
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Digital_Transformation.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // One prompt paragraph, with the "Scenario:" label kept but re-punctuated as "Scenario — ",
    // for the reason recorded in digital-transformation-build.js — Coursera rejected the three
    // Module 1 questions that carried the colon. This must match the builder exactly.
    ...
    // option, which is what rejected Q2, Q8 and Q9 of Module 1 when the label was kept. A
    // multi-word opening that happens to contain a colon is prose; see the note in
    // digital-transformation-build.js. It is reported, not failed.
    ...
    gotPrompt.forEach((p, i) => {
      ...
      if (one) {
        ...
        console.log(`   NOTE ${tag} prompt line ${i + 1} opens the one-word label "${one[1]}:" — kept `
          + 'verbatim by decision; this is the failure Q2, Q8 and Q9 already hit');
      } else if (label)
    ...
    const shape = blk.map(l =>
      ...
      : /^Feedback:\s/.test(l) ? 'F'
      : 'P');
    // Always 1: the builder joins every prompt to a single paragraph.

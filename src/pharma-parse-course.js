...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    // All four objectives sit in ONE bulleted, <w:br/>-separated paragraph, so each line is
    // matched separately rather than the paragraph as a whole.
    if (!inPart2 && /(?:^|\n)\s*(?:•\s*)?LO\d+\s*[:\-–]/i.test(t)) {
      let hit = false;
      for (const line of t.split('\n')) {
        const lm = line.match(/^\s*(?:•\s*)?(LO\d+)\s*(?:\([^)]*\))?\s*[:\-–]\s*(.+)$/i);
        if (lm) { courseLOs[lm[1].toUpperCase()] = lm[2].trim(); hit = true; }
      }
      if (hit) continue;
    }

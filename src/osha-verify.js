...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_OSHA.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    src.options.forEach((o, oi) => {
      ...
      // required shape: ONE line — "Feedback: <explanation> (Refer to Module ...)"
      ...
      if (fbs[oi] !== wantFb) {
        ...
        if (!/^Refer to Module \d+ Lesson \d+ Video: \S/.test(wantRef))
        ...
        if (!/\(Refer to Module \d+ Lesson \d+ Video: .+\)$/.test(fbs[oi]))

...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'ai-products', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors ai-products-build.js. This source carries the verdict inside the explanation's
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_AI_Products.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // ai-products-build.js. It is reported, not failed.

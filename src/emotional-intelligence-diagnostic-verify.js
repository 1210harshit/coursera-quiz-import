...
const mods = JSON.parse(fs.readFileSync(path.join(SP, 'emotional-intelligence', 'diagnostic.json'), 'utf8'));
const { meta: MMETA } = JSON.parse(fs.readFileSync(path.join(SP, 'emotional-intelligence', 'outline.json'), 'utf8'));
...
// The diagnostic references the MODULE a question is drawn from, not a video — see
// emotional-intelligence-diagnostic-build.js. The two must produce byte-identical lines.
const referLine = n => {
  const v = MMETA['M' + n];
  return `Refer to Module ${n}: ${v.title}`;
};
...
for (const mo of mods) {
  const file = path.join(OUT, 'Coursera_Import_Pre_Course_Diagnostic_Emotional_Intelligence.docx');
  console.log(`\n=== ${mo.title}: ${path.basename(file)} ===`);
  ...
  idx.forEach((start, k) => {
    ...
    const tag = `Q${src.num}`;
    const wantRef = referLine(src.module);
    ...
    src.options.forEach((o, oi) => {
      ...
      if (fbs[oi] !== wantFb) {
        ...
        if (!/^Refer to Module \d+: \S/.test(wantRef))
        ...
        // A module title may itself contain brackets or a colon ("The Essentials: Awareness,
        // Management, Empathy, and Influence"), so the wrapper may legitimately nest and the title is
        // matched loosely. Require the wrapper, not the absence of nesting.
        if (!/\(Refer to Module \d+: .+\)$/.test(fbs[oi]))
    ...
    // One paragraph, joined with a space — mirrors emotional-intelligence-diagnostic-build.js, which must
    // emit a single prompt line or the whole document fails to import. See the note there.
    ...
    gotPrompt.forEach((p, i) => {
      ...
      if (label && !/\s/.test(label[1]))
        bad(`${tag} prompt paragraph ${i + 1} starts with a one-word label: "${p.slice(0, 40)}"`);
      else if (label)
    ...
    const shape = blk.map(l =>
      ...
      : /^Feedback:\s/.test(l) ? 'F'
      : 'P');
    // Always 1: the builder joins every prompt into a single paragraph. A shape of HPP… is the
    // defect that made Coursera report no importable questions at all.

...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'genai-pm', 'outline.json'), 'utf8'));
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_GenAI_PM.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // genai-pm-build.js. It is reported, not failed.

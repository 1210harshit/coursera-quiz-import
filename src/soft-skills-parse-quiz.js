// Parser for the Soft Skills for Work and Life graded assessments (one file per module).
//
...
for (const n of MODULES) {
  ...
  if (!fs.existsSync(src)) {
    console.error(`ENOENT ${src}\nUnzip the module ${n} graded quiz first:\n`
      + `  unzip -q "SoftSkillsDummies_M${n}_GradedQuiz.docx" -d work/${SLUG}/m${n}`);
    process.exit(1);
  }
  ...
  starts.forEach((start, k) => {
    ...
    if (SCENARIO.test(head[2]) || !head[2]) {
      ...
      // builder decides what reaches the import section — see soft-skills-build.js, which joins

...
const SP = __dirname;
const xml = fs.readFileSync(path.join(SP, 'cstp-course-1', 'outline', 'word', 'document.xml'), 'utf8');
...
for (const b of blocks) {
  if (b.type === 'p') {
    ...
    // Instructor is stated once for the whole series, before the Course 1 heading.
    ...
    // Module 2 leaves the value blank and puts "C1LO2 - ..." on the next line.
    if ((m = t.match(/^(?:•\s*)?Aligned Course-level Learning Objective\s*:\s*(C1LO\d+)?/i)) && mod) {
      if (m[1]) meta['M' + mod].alignedLOs = [m[1]];
      else meta['M' + mod]._awaitLO = true;
      continue;
    }
    if ((m = t.match(/^(C1LO\d+)\b/i)) && mod && meta['M' + mod]._awaitLO) {
      meta['M' + mod].alignedLOs = [m[1]];
      meta['M' + mod]._awaitLO = false;
      continue;
    }

...
for (const z of quizzes) {
  const NAME = fileFor(z);
  const file = path.join(OUT, NAME);
  console.log('\n=== Module ' + z.module + ': ' + NAME + ' ===');
  ...
  for (const part of ['[Content_Types].xml', '_rels/.rels', 'word/document.xml',
    ...
    if (!fs.existsSync(path.join(ex, part))) bad('missing package part ' + part);
  }
  ...
  const secPairs = lines.slice(s + 1, e).map((l, i) => ({ l, x: xmls[s + 1 + i] })).filter(q => q.l);
  const sec = secPairs.map(q => q.l);

  secPairs.forEach((q, i) => {
    if (/<w:br\b/.test(q.x)) bad('line break inside import section at line ' + i + ': "' + q.l.slice(0, 60) + '"');
    for (const [re, nm] of FMT) {
      if (re.test(q.x)) bad(nm + ' formatting inside import section at line ' + i + ': "' + q.l.slice(0, 60) + '"');
    }
  });

  // The source's own scaffolding must not survive into the imported region: the mapping line,
  // and the bare "Correct" / "Feedback" row labels that carried the answer key.
  sec.forEach(l => {
  ...
  // Each document numbers its own questions 1..N. The source already does, but a document that
  // silently inherited the other split would show up here.
  ...
  idx.forEach((start, k) => {
    ...
    const tag = 'M' + z.module + ' Q' + src.num;
  ...
  // The outline states one course-level budget; each module document carries its share.
  const mins = Math.max(5, Math.round(COURSE_MINS * z.questions.length / TOTAL_Q));
  const wantTime = String(Math.floor(mins / 60)).padStart(2, '0') + ':' + String(mins % 60).padStart(2, '0');
  if (after('Time Estimate (hh:mm)') !== wantTime) {
    bad('Time Estimate is "' + after('Time Estimate (hh:mm)') + '", expected "' + wantTime +
        '" (' + COURSE_MINS + ' min course budget over ' + TOTAL_Q + ' questions)');
  }

  // Each document must name its OWN module — a copy-paste of another module's text would show
  // up here rather than at import.
  const meta = MMETA['M' + z.module] || { alignedLOs: [], lessons: {} };
  const modTitle = meta.title || z.moduleTitle;
  if (!guideText.includes('Module ' + z.module + ' — ' + modTitle)) {
    bad('guide does not identify itself as Module ' + z.module + ' — ' + modTitle);
  }
  for (const other of quizzes) {
    if (other.module === z.module) continue;
    const otherTitle = (MMETA['M' + other.module] || {}).title || other.moduleTitle;
    if (otherTitle && guideText.includes('Module ' + other.module + ' — ' + otherTitle)) {
      bad('guide also names Module ' + other.module + ' — content from another module leaked in');
    }
  }

  // Only this module's aligned course objective, and it must be the right one.
  const wantLOs = (meta.alignedLOs || []).filter(id => COURSE.los[id]);
  for (const id of wantLOs) {
    if (!guide.some(l => l.startsWith(id + ':'))) bad('aligned objective ' + id + ' missing from the guide');
  }
  for (const id of Object.keys(COURSE.los)) {
    if (!wantLOs.includes(id) && guide.some(l => l.startsWith(id + ':'))) {
      bad('guide lists ' + id + ', which is not this module\'s aligned objective');
    }
  }

  // Traceability table must account for every question.
  for (const q of z.questions) {
    if (!guide.includes(q.mapped)) bad('traceability row missing the mapping ' + q.mapped);
  }
  ...
  console.log('   ' + idx.length + ' questions numbered 1-' + idx.length + ' | ' + refCount +
    ...
    new Set(used).size + ' live hyperlinks | ' + wantLOs.length + ' objective | ' +
    wantTime + ' time estimate | package OK');
  fs.rmSync(ex, { recursive: true, force: true });
}

console.log(fail === 0
  ? '\n✅ ALL CHECKS PASSED — ' + quizzes.length + ' files (one per module), ' + totalQ +
    ' questions, ' + totalFb + ' feedback blocks, references verified against the outline.'
  : '\n❌ ' + fail + ' CHECK(S) FAILED');

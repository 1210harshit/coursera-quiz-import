...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'soft-skills', 'outline.json'), 'utf8'));
const dec = s => s.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"')
                  .replace(/&apos;/g, "'").replace(/&amp;/g, '&');

// Mirrors soft-skills-build.js. This source carries the verdict inside the explanation's
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Soft_Skills.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // "Scenario — ". Mirrors soft-skills-build.js; the two must stay identical or this fails a
    ...
    // soft-skills-build.js. It is reported, not failed.

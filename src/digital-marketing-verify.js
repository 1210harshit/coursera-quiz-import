...
const { map: VIDEOS } = JSON.parse(fs.readFileSync(path.join(SP, 'digital-marketing', 'outline.json'), 'utf8'));
...
for (const mo of mods) {
  const file = path.join(OUT, `Coursera_Import_Module_${mo.num}_Graded_Quiz_Digital_Marketing.docx`);
  ...
  idx.forEach((start, k) => {
    ...
    // Compared EXACTLY — no whitespace collapsing on either side — so a run of spaces the
    // author wrote has to survive into the document.
